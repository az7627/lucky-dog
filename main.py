...
def open_options():
    ...
    ttk.Button(right_frame, text="更新", command=update_selected).pack(pady=10)
    ...
    font_combo = ttk.Combobox(frame_settings, textvariable=font_var, values=fonts, state='readonly')
    ...
    ttk.Checkbutton(frame_settings, text="开启动画", variable=anim_var).grid(row=2, column=0, columnspan=2, sticky=tk.W, padx=5, pady=5)
    ...
    ttk.Button(left_buttons, text="添加", command=add_student).pack(side=tk.LEFT, padx=5)
    ttk.Button(left_buttons, text="删除", command=delete_student).pack(side=tk.LEFT, padx=5)
    ...
    def apply_changes():
        ...
        new_config = {
            ...
            "animation_enabled": anim_var.get(),
            "animation_steps": steps_var.get(),
            "animation_interval": interval_var.get()
        }

        # 保存配置到文件
        save_config_to_file(new_config)

        # 保存原始名单到文件（使用 original_names）
        ...
        messagebox.showinfo("成功", "设置已应用，新名单已生效。")
...
pickup_button = tk.Button(mainWindow, text="选取名字", font=(font_family, 40),
